...
* **New:** A **Delete Old Threads** button in the Discord options. Discord's own thread UI cannot tell Ground Station session threads from human or other-bot threads, cannot bulk-delete only those threads together with their parent-channel starter messages, and cannot leave direct-to-channel traffic untouched. This tool deletes only Ground Station–created session threads older than a configurable number of days, including the top-level channel messages that anchor them. Messages posted directly to the channel are kept. A progress dialog shows each thread as it is deleted, the estimated time remaining, and allows cancellation.
