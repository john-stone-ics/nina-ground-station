...
* **New:** A **Delete Old Threads** button in the Discord options removes session threads older than a configurable number of days, including the top-level channel messages that anchor them. Only threads created by Ground Station are deleted; messages posted directly to the channel are kept. A progress dialog shows each thread as it is deleted, the estimated time remaining, and allows cancellation.
